...
def make_model_small(vocab_size=6, embed_dim=32, latent_d_model=64, nhead=4):
    ...
    encoder_layer_configs = [{"window_size": 16, "r": 2}, {"window_size": 16, "r": 2}, {"window_size": 16, "r": 3}, {"window_size": 16, "r": 3}]
    decoder_layer_configs = [{"window_size": 16}] * 2

    local_enc = LocalEncoder(vocab_size=vocab_size, d_model=embed_dim, nhead=nhead, layer_configs=encoder_layer_configs)
    ...
    local_dec = LocalDecoder(d_model=embed_dim, vocab_size=vocab_size, layer_configs=decoder_layer_configs)
