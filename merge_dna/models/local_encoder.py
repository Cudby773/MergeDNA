...
class LocalMergeBlock(nn.Module):
    ...
    Returns:
        ...
        merge_scores: (B, L) 
    """
    def __init__(self, d_model: int, nhead: int, window_size: int, r: int):
        """
        ...
        self.r = r
        self.token_merge = TokenMerge(r)
        self.M_w = window_size - r
        
        
    def build_source_index(self, source_map_windows: torch.Tensor, B: int, W: int, num_windows: int, L_orig: int, device):
        source_map_windows = source_map_windows.view(B, num_windows, W)  # (B, num_windows, W)
        offsets_per_window = (torch.arange(num_windows, device=device, dtype=torch.long) * self.M_w).view(1, num_windows, 1)
        source_index_global_windows = source_map_windows + offsets_per_window  # (B, num_windows, W)
...
class LocalEncoder(nn.Module):
    ...
    def __init__(self, vocab_size: int, d_model: int, nhead: int, layer_configs: list[dict]):
        ...
        for cfg in layer_configs:
            ...
            block = LocalMergeBlock(d_model=d_model, nhead=nhead, window_size=W, r=r)
